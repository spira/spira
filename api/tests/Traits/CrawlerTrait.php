...
/**
 * With the future release of Lumen 5.1, this trait can most probably be
 * heavily reduced, as it seems like much of this functionality will be
 * available within the 5.1 release.
 */
trait CrawlerTrait
{
    /**
    ...
    {
        $actual = json_encode($this->array_sort_recursive(
            json_decode($this->response->getContent(), true)
        ));

        $this->assertEquals(json_encode($this->array_sort_recursive($data)), $actual);
    ...
    protected function seeJsonContains(array $data)
    {
        ...
        foreach ($this->array_sort_recursive($data) as $key => $value) {
     ...
     */
    public function call2($method, $uri, $parameters = [], $cookies = [], $files = [], $server = [], $content = null)
    {
        $this->currentUri = $this->prepareUrlForRequest($uri);
        // var_dump($this->currentUri);

        $this->response = parent::call($method, $this->currentUri, $parameters, $cookies, $files, $server, $content);
