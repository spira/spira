...
$factory->define(App\Models\History::class, function (\Faker\Generator $faker) {
    $historable_type = $faker->randomElement(['Article']);

    switch ($historable_type) {
        case 'Article':
            $parent_id = null;
            $changes = [
                'title' => [
                    'before' => $faker->sentence,
                    'after' => $faker->sentence
                ],
                'content' => [
                    'before' => $faker->paragraph,
                    'after' => $faker->paragraph
                ],
                'excerpt' => [
                    'before' => $faker->sentence,
                    'after' => $faker->sentence
            ...
            $keys = $faker->randomElements(array_keys($changes), $faker->numberBetween(1, count($changes)));
            $changes = array_only($changes, $keys);
            break;
    ...
    return [
        'history_id' => $faker->uuid,
        'historable_id' => $faker->uuid,
